...
export default async function HomePage() {
  ...
  // Pre-fetch latest digest for each active country (responses cached 1h by Next.js)
    ...
    <div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
          <div className="max-w-2xl">
            ...
            <p className="text-lg text-slate-300 leading-relaxed mb-8">
              AI-curated daily digests from leading economic media across Latin America and beyond.
              Stay informed with concise, sourced summaries.
            </p>
            <div className="flex flex-wrap gap-3">
              {activeCountries.map((c) => {
                const latest = latestBySlug[c.slug]
                return (
                  <Link
                    key={c.slug}
                    href={`/${c.slug}`}
      ...
      <section id="countries" className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16">
        <div className="flex items-center justify-between mb-8">
          ...
          <span className="text-sm text-slate-500">{countries.length} countries · {activeCountries.length} active</span>
        </div>
                ...
                {latest ? (
                  <>
                    ...
                    <p className="text-xs text-slate-500 mb-4">
                      {latest.articleCount} artículos · {latest.sources.join(', ')}
                    </p>
      ...
      <section className="bg-slate-900 text-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            {[
              { icon: '🤖', label: 'AI-Powered', desc: 'Articles selected and summarized by AI daily' },
              { icon: '📰', label: 'Trusted Sources', desc: 'Curated from leading economic media outlets' },
              { icon: '🌎', label: 'Global Coverage', desc: 'Multi-country, bilingual (ES/EN) platform' },
            ].map(({ icon, label, desc }) => (
